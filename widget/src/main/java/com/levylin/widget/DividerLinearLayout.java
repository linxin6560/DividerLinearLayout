 ...
 * {@link LayoutParams DividerLinearLayout.LayoutParams}.
 ...
 * <p>
 * <p>
 * 根据{@link android.support.v7.widget.LinearLayoutCompat}修改，
 * 所以使用方法也和{@link android.support.v7.widget.LinearLayoutCompat}类似
 * 在xml中使用：
 * android:orientation="vertical" //和LinearLayout的一样
 * app:d_divider="@drawable/divider" //设置分割线，分割线要设置高度或者宽度，否则无效
 * app:d_innerDividerPaddingLeft="14dp"//设置内部分割线的左间距
 * app:d_innerDividerPaddingRight="14dp"//设置内部分割线的右间距
 * app:d_showDividers="middle|end|beginning"//设置显示分割线的位置：middle:两个View之间，end：底部分割线，beginning：头部分割线
 * <p>
 * <p>
 * 暂不支持底部和头部的分割线设置左右间距。。。。。如需求量多再考虑加入
 *
 * @author luoxian
 */
@SuppressLint("RestrictedApi")
public class DividerLinearLayout extends ViewGroup {
    /**
     * @hide
     */
    ...
    public DividerLinearLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        final TintTypedArray a = TintTypedArray.obtainStyledAttributes(context, attrs,
                R.styleable.DividerLinearLayout, defStyleAttr, 0);

        int index = a.getInt(R.styleable.DividerLinearLayout_android_orientation, -1);
        if (index >= 0) {
            setOrientation(index);
        }

        index = a.getInt(R.styleable.DividerLinearLayout_android_gravity, -1);
        if (index >= 0) {
            setGravity(index);
        }

        boolean baselineAligned = a.getBoolean(R.styleable.DividerLinearLayout_android_baselineAligned, true);
        if (!baselineAligned) {
            setBaselineAligned(baselineAligned);
        }

        mWeightSum = a.getFloat(R.styleable.DividerLinearLayout_android_weightSum, -1.0f);

        mBaselineAlignedChildIndex =
                a.getInt(R.styleable.DividerLinearLayout_android_baselineAlignedChildIndex, -1);

        mUseLargestChild = a.getBoolean(R.styleable.DividerLinearLayout_d_measureWithLargestChild, false);

        setDividerDrawable(a.getDrawable(R.styleable.DividerLinearLayout_d_divider));
        mShowDividers = a.getInt(R.styleable.DividerLinearLayout_d_showDividers, SHOW_DIVIDER_NONE);
        mDividerPaddingLeft = a.getDimensionPixelSize(R.styleable.DividerLinearLayout_d_innerDividerPaddingLeft, 0);
        mDividerPaddingRight = a.getDimensionPixelSize(R.styleable.DividerLinearLayout_d_innerDividerPaddingRight, 0);
    ...
    /**
     * Set left padding displayed on both ends of dividers.
     *
    ...
    /**
     * Set right padding displayed on both ends of dividers.
     *
     * @param padding Padding value in pixels that will be applied to each end
     * @see #setShowDividers(int)
     * @see #setDividerDrawable(Drawable)
    ...
    void drawDividersVertical(Canvas canvas) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (child != null && child.getVisibility() != GONE) {
                if (hasDividerBeforeChildAt(i)) {
                    ...
                    drawHorizontalDivider(canvas, top, i);
                }
            }
        }
        ...
        if (hasDividerBeforeChildAt(count)) {
            ...
            if (lastVisibleView == null) {
                ...
                final LayoutParams lp = (LayoutParams) lastVisibleView.getLayoutParams();
                bottom = lastVisibleView.getBottom() + lp.bottomMargin;
            }
            drawHorizontalDivider(canvas, bottom, count);
        }
    }
    ...
    void drawDividersHorizontal(Canvas canvas) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (child != null && child.getVisibility() != GONE) {
                if (hasDividerBeforeChildAt(i)) {
                    final LayoutParams lp = (LayoutParams) child.getLayoutParams();
                    final int position = child.getLeft() - lp.leftMargin - mDividerWidth;
                    drawVerticalDivider(canvas, position, i);
                }
            }
        }
        ...
        if (hasDividerBeforeChildAt(count)) {
            ...
            if (lastVisibleView == null) {
                ...
                final LayoutParams lp = (LayoutParams) lastVisibleView.getLayoutParams();
                position = lastVisibleView.getRight() + lp.rightMargin;
            }
            drawVerticalDivider(canvas, position, count);
        }
    }

    void drawHorizontalDivider(Canvas canvas, int top, int position) {
        int left = getPaddingLeft();
        int right = getWidth() - getPaddingRight();
        if (isMiddle(position)) {
            left = left + mDividerPaddingLeft;
            right = right - mDividerPaddingRight;
        }
        mDivider.setBounds(left, top, right, top + mDividerHeight);
        mDivider.draw(canvas);
    }

    void drawVerticalDivider(Canvas canvas, int left, int position) {
        int top = getPaddingTop();
        int bottom = getHeight() - getPaddingBottom();
        if (isMiddle(position)) {
            top = top + mDividerPaddingLeft;
            bottom = bottom - mDividerPaddingRight;
        }
        mDivider.setBounds(left, top, left + mDividerWidth, bottom);
        mDivider.draw(canvas);
    }
     ...
     * @return
     */
    private boolean isMiddle(int position) {
    ...
    public void onInitializeAccessibilityEvent(AccessibilityEvent event) {
        ...
        event.setClassName(DividerLinearLayout.class.getName());
    }

    @Override
    public void onInitializeAccessibilityNodeInfo(AccessibilityNodeInfo info) {
        super.onInitializeAccessibilityNodeInfo(info);
        info.setClassName(DividerLinearLayout.class.getName());
    }
    ...
    public static class LayoutParams extends MarginLayoutParams {
        ...
        public LayoutParams(Context c, AttributeSet attrs) {
            super(c, attrs);
            TypedArray a = c.obtainStyledAttributes(attrs, R.styleable.DividerLinearLayout_Layout);

            weight = a.getFloat(R.styleable.DividerLinearLayout_Layout_android_layout_weight, 0);
            gravity = a.getInt(R.styleable.DividerLinearLayout_Layout_android_layout_gravity, -1);
